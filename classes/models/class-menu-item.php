...
class Menu_item extends Store_item {
	...
	public function get_menu_items_by_args( array $args ) {
		...
		// Shortcode / widget
		$tags_ids     = ! empty( $args[ 'tags_list' ] ) ? $this->get_taxonomies_ids( $args, 'tags_list' ) : array();
		$category_ids = ! empty( $args[ 'categ' ] ) ? $this->get_taxonomies_ids( $args, 'categ' ) : array();
		
		// Single taxonomy view
		$taxonomy_ids = ! empty( $args[ 'taxonomy' ] ) ? $this->get_taxonomies_ids( $args ) : array();
		
		
		if ( ! empty( $category_ids ) && ! empty( $tags_ids ) ) {
			
			foreach ( $category_ids as $id ) {
				$term_params             = array_merge( $params, array(
						'tax_query' =>
							array(
								'relation' => 'AND',
								array(
			...
			$items = $this->get_menu_items_by_taxonomy( $category_ids, $params, $items );
		} elseif ( ! empty( $tags_ids ) ) {
			$items = $this->get_menu_items_by_taxonomy( $tags_ids, $params, $items );
		} elseif ( ! empty( $taxonomy_ids ) ) {
			$items = $this->get_menu_items_by_taxonomy( $taxonomy_ids, $params, $items );
		} else {
			$items[] = array( 'term' => '', 'posts' => get_posts( $params ) );
		}
		
		return $this->get( 'menu_category' )->sort_taxonomy_order( $items );
	}
	 ...
	 * @return array
	 */
	protected function get_taxonomies_ids( array $args, $key_args = 'taxonomy' ) {
		$taxonomy_ids = array();
		if ( ! empty( $args[ $key_args ] ) ) {
			
			if ( is_array( $args[ $key_args ] ) ) {
				
				$taxonomy_ids = $args[ $key_args ];
				
			} elseif ( isset( $args[ $key_args ]->term_id ) && ( $args[ $key_args ] instanceof \WP_Term ) ) {
				
				$taxonomy_ids = array( $args[ $key_args ]->term_id );
				
			} else {
				
				$taxonomy_ids = explode( ',', $args[ $key_args ] );
	 ...
	 * @return array
	 */
	protected function get_menu_items_by_taxonomy( $taxonomy_ids, $params, $items ) {
		...
		foreach ( $taxonomy_ids as $id ) {
			...
			$items[ $id ]            = array( 'term' => $term, 'posts' => get_posts( $term_params ) );
			$items[ $id ][ 'order' ] = $this->get( 'menu_category' )->get_term_order( $id );
		}
