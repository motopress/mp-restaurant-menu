...
//				$limit = edd_get_file_download_limit($menu_item['id']);
//				if (!empty($limit)) {
//					edd_set_file_download_limit_override($menu_item['id'], $purchase_id);
//				}
//			}
//		}
	...
	public function admin_email_notice($payment_id = 0, $payment_data = array()) {
		...
		$subject = $this->get('settings')->get_option('sale_notification_subject', sprintf(__('New download purchase - Order #%1$s', 'mp-restaurant-menu'), $payment_id));
	...
	function get_sale_notification_body_content($payment_id = 0, $payment_data = array()) {
		...
		$menu_item_list = '';
		$menu_items = maybe_unserialize($payment_data['downloads']);
		if (is_array($menu_items)) {
			foreach ($menu_items as $menu_item) {
