<?php do_action('mprm_purchase_link_form_before', $post->ID, $args) ?>
	<div class="mprm-display-inline">
		<?php mprm_get_preloader('mprm-hidden'); ?>
		<form id="<?php echo $data['form_id']; ?>" class="mprm_purchase_form mprm_purchase_<?php echo $post->ID; ?>" data-id="<?php echo $post->ID ?>" method="post">
			...
			<div class="mprm_purchase_submit_wrapper">
				...
				if (!$is_ajax_disabled) { ?>
					<a href="#" class="mprm-add-to-cart mprm-has-js <?php echo esc_attr($class) ?> mprm-display-inline" data-action="mprm_add_to_cart" data-menu-item-id="<?php echo esc_attr($post->ID) ?>" <?php echo $data_variable . ' ' . $type . ' ' . $data_price . ' ' . $button_display ?>>
				...
				<?php if (!$is_ajax_disabled) : ?>
					...
					<span class="mprm-cart-added-alert" style="display: none;">	<i class="mprm-icon-ok"></i> <?php _e('Added to cart', 'mp-restaurant-menu'); ?></span>
				</span>
				<?php endif; ?>
