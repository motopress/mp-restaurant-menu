 ...
 *
 */
function mprm_email_tag_shipping_address( $order = false ) {
	$shipping_address_html = '';
	if ( ! empty( $order ) && is_integer( $order ) ) {
		
		$order_object     = mprm_get_order_object( get_post( $order ) );
		$shipping_address = empty( $order_object->shipping_address ) ? '' : $order_object->shipping_address;
		
		if ( ! empty( $shipping_address ) ) {
			$shipping_address_html = View::get_instance()->get_template_html( 'emails/tags/shipping-address', array( 'shipping_address' => apply_filters( 'mprm_email_tag_shipping_address', $shipping_address ) ) );
		}
	}
	
	return apply_filters( 'mprm_email_tag_shipping_address_html', $shipping_address_html );
}
 ...
 *
 */
function mprm_email_tag_order_notes( $order = false ) {
	$order_notes_html = '';
	if ( ! empty( $order ) && is_integer( $order ) ) {
...
/**
 * Default sale notification email
 *
 * @return mixed|string
 */
